...
// Drag / merge / item chrome live in sortable.js (Replace extraction).
...
const {
  ...
  HANDLE_CLASS,
  EDIT_CLASS,
  getItem: sortableGetItem,
} = sortable

// Back-compat `story` surface for editor.js (was refresh IIFE exports).
const story = sortable


const pageModule = require('./page')
const { newPage, asSlug, pageEmitter } = pageModule

const getItem = sortableGetItem
...
const emitControls = $journal => {
  ...
  // Keep inside .journal (stock layout).
  $journal.append(`\
...
const renderPageIntoPageElement = function (pageObject, $page) {
  ...
  const [$handleParent, $twins, $header, $story, $backlinks, $journal, $footer] = [
    ...
    if (className !== 'journal' || story.isEditOn()) {
  ...
  // Drag strip: paper top → just above the title. Twins stay clickable via CSS
  // pointer-events (empty chrome around them still starts a page drag).
  const headerTop = $header?.position?.()?.top
  const handleTop = $handleParent.position().top
  const handleBottom =
    typeof headerTop === 'number' ? headerTop : $story.position().top
  $pagehandle.css({
    height: `${Math.max(24, handleBottom - handleTop)}px`,
  })
  return promise
}
...
module.exports = {
  ...
  JOURNAL_SELECTED_CLASS,
  shouldSuppressJournalClick,
  setStorySortableEnabled: story.setStorySortableEnabled,
  ensure: story.ensure,
  clear: story.clear,
  select: story.select,
  isEditOn: story.isEditOn,
  needsDragHandle: story.needsDragHandle,
  HANDLE_CLASS: story.HANDLE_CLASS,
  EDIT_CLASS: story.EDIT_CLASS,
  getItem: story.getItem,
  getSelectedId: story.getSelectedId,
  shouldSuppressItemClick: story.shouldSuppressItemClick,
}
