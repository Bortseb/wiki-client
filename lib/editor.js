...
var textEditor = function ($item, item, option) {
  ...
  const focusoutHandler = function () {
    $item.removeClass('textEditing').css('marginBottom', '')
    $textarea.off()
  ...
  $('.item.textEditing textarea').trigger('focusout')

  const el = $item[0],
    next = $item.nextAll('.item')[0],
    height = el.getBoundingClientRect().height || 150,
    gap = next ? Math.max(0, next.offsetTop - el.offsetTop - el.offsetHeight) : 0,
    $page = $item.parents('.page:first'),
    scrollTop = $page.scrollTop()

  $item.addClass('textEditing').off()
  if (gap) $item.css('marginBottom', gap)
  ...
  var $textarea = $(`<textarea>${escape(original)}${escape(option.suffix || '')}</textarea>`)
    ...
    .on('input', function () {
      ...
      const chrome = this.offsetHeight - this.clientHeight
      this.style.height = Math.max(height, this.scrollHeight) + chrome + 'px'
      if (this.scrollHeight > this.clientHeight) {
        this.style.height = this.scrollHeight + chrome + 'px'
      }
    })
  $item.html($textarea.trigger('input'))
  if (option.caret) {
