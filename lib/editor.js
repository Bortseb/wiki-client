...
var textEditor = function ($item, item, option) {
  ...
  const focusoutHandler = function () {
    ...
    $textarea.off()
    if (document.getElementsByClassName("textEditing").length === 0) window.removeEventListener("beforeunload", beforeUnloadHandler);
  ...
  var $textarea = $(`<textarea>${escape(original)}${escape(option.suffix || '')}</textarea>`)
    // .on('focusout', focusoutHandler) // The previous way to handle focusout
