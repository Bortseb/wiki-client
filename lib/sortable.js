...
// Ghost handle-reorder is DOM-only (no put / no implicit fork). Explicit fork
// copies this on-screen order into the page JSON so the new local page matches.
const syncStoryOrderFromDom = function ($page) {
  if (!$page?.length) return false
  const pageObject = lineup.atKey($page.data('key'))
  const page = pageObject?.getRawPage?.()
  if (!page?.story) return false
  const order = getStoryItemOrder($page.find('.story').first())
  if (!order.length) return false
  const current = page.story.map(item => item?.id)
...
const handleDrop = function (evt, $item, originalIndex, originalOrder, opts = {}) {
  ...
  if (moveWithinPage) {
    ...
    if (JSON.stringify(order) !== JSON.stringify(originalOrder)) {
      ...
      // Ghost reorder is session-only — putting a move would implicit-fork.
...
const placeItemOnPage = function ($item, $destPage, opts = {}) {
  ...
  // Cross-page onto a ghost stays no-drop; same-page local reorder matches the handle path.
  if ($destPage.hasClass('ghost') && !samePage) return
  ...
  // Same-page move = reorder only (no alias / add). Ignore sourceIsReadOnly —
  // ghost same-page reorder is DOM-local until fork (putMove no-ops on ghost).
  if (samePage && !wantCopy) {
    insertNode($item)
    clearContent($item)
    let index = $('.item').index($item)
    if (originalIndex >= 0 && originalIndex < index) index = originalIndex
    plugin.renderFrom(index)
    putMove($destPage, $story, item.id)
    return
  }
...
const initStorySortable = function ($page) {
  ...
  const sortable = Sortable.create(storyEl, {
    ...
    onStart(evt) {
      ...
      try {
        require('./active').markChosen()
      } catch (_) {
        /* optional until Honor lands upstream */
      }
      try {
...
//   (same-page drop, including on a ghost, is local reorder — same as the handle)
...
const bindLineupItemDrag = function () {
  ...
  const updateHoverUi = function () {
    ...
    // Ghost dest is no-drop across pages; same-page reorder matches the handle path.
    if (!pageEl || (pageEl.classList.contains('ghost') && !sameSourcePage)) {
  ...
  const onDragCommit = function (e) {
    ...
    const sameSourcePage = !!(
      $source?.length &&
      $dest?.length &&
      equals($source, $dest)
    )
    teardown()
    // Cross-page onto a ghost stays invalid; same-page local reorder is allowed.
    if (!didMove || !$item?.length || !$dest?.length || ($dest.hasClass('ghost') && !sameSourcePage)) {
      select($item)
