...
const bind = function () {
  ...
  // Stacked → wrap site controls in .footer-content-row; unstacked → unwrap (keeps footer > #security).
  const syncContentRow = stacked => {
  ...
  const syncChevrons = () => {
    ...
    // Measure the origin .neighbor span (not the spinning img): rotate transforms inflate
    // the image AABB and make chevrons jitter while the neighborhood is loading.
    ...
    const heightPx = 16 // matches .neighbor img; keep steady while .wait/.fetch rotate
    const topPx = flagRect
      ? Math.round(flagRect.top - fr.top - borderTop + (flagRect.height - heightPx) / 2)
  ...
  const layout = () => {
    ...
    const needed = footerContentWidth(pagesW, (mobile ? 2.25 : 10) * em) + flagWidth()
