...
const bind = function () {
  ...
  const syncChevrons = () => {
    ...
    const flag = track?.querySelector('.neighbor')
    const flagH = flag?.offsetHeight || 16
    const padTop = parseFloat(getComputedStyle(hood).paddingTop) || 0
    const padBot = parseFloat(getComputedStyle(hood).paddingBottom) || 0
    const innerH = Math.max(flagH, hood.clientHeight - padTop - padBot)
    const top = `${Math.round(box.top - fr.top + padTop + (innerH - flagH) / 2)}px`
    const height = `${flagH}px`
      ...
      if (child === controls) {
        for (const piece of controls.children) {
          if (search && piece.contains?.(search)) {
            total += searchW + pagesW + (parseFloat(getComputedStyle(piece).gap) || 6)
          } else {
            total += ow(piece)
          }
        }
      } else {
        total += ow(child) || child.scrollWidth || 0
      }
    }
    if (!row && !isMobile()) total += 12 // .footer-item margins
    return total + gap * n
  }
  ...
  const layout = () => {
    ...
    const stacked = chromeWidth(pagesW, (mobile ? 2.25 : 10) * em) + flagWidth() > available + 0.5
  ...
  if (footer && typeof ResizeObserver === 'function') {
    new ResizeObserver(scheduleLayout).observe(footer)
  }
