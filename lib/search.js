...
const createSearch = function ({ neighborhood }) {
  const placeIncrementalSearch = $el => {
    ...
    const stacked = footer?.classList.contains('neighborhood-above') && hood
    const mobile = window.matchMedia('(max-width: 490px)').matches
    const anchorTop = (stacked ? hood : $searchbox[0]).getBoundingClientRect().top

    if (mobile || stacked) {
      const searchRect = $searchbox[0].getBoundingClientRect()
      if (!$el.parent().is('body')) $el.appendTo(document.body)
      $el.css({
        position: 'fixed',
        left: mobile ? '0' : `${Math.round(searchRect.left)}px`,
        right: mobile ? '0' : 'auto',
        width: mobile ? '100%' : 'auto',
        maxWidth: mobile ? 'none' : '',
        bottom: `${Math.max(0, window.innerHeight - anchorTop + 6)}px`,
        boxSizing: 'border-box',
      })
      return
    }

    if (!$el.parent().is($searchbox)) $el.appendTo($searchbox)
    ...
    $el.css({
      ...
      width: 'auto',
      maxWidth: '',
      bottom: `${offset.top + $searchbox.height()}px`,
    })
  }
