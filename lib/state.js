...
state.show = () => {
  ...
  // Dummy history entries (mobile editor/search) popstate with the same
  // lineup. Focusing .page:last here jumped to the end of the lineup.
  const unchanged =
    oldPages.length === newPages.length && oldPages.every((name, i) => name === newPages[i])
  if (!unchanged) {
