...
`)
  ...
  $('<span>&nbsp; wiki <span class=editEnable>✔︎</span> &nbsp; </span>')
    ...
    .appendTo('footer')
    .on('click', function () {
      const enable = !$('.editEnable').is(':visible')
      $('.editEnable').toggle(enable)
      localStorage.setItem('wikiEditEnable', enable)
      $('.page').each(function () {
        const $page = $(this)
        const pageObject = lineup.atKey($page.data('key'))
        refresh.rebuildPage(pageObject, $page.empty())
  ...
  const saved = localStorage.getItem('wikiEditEnable')
  $('.editEnable').toggle(
    saved != null ? saved === 'true' : isAuthenticated && !matchMedia('(max-width: 490px)').matches,
  )
